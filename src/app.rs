...
pub struct App {
    ...
    /// Index of selected tag in sidebar
...
impl App {
    ...
    pub fn select_previous_project(&mut self) {
        ...
        self.update_filter_for_project();
    }
    ...
    /// Updates the filter based on selected project.
    fn update_filter_for_project(&mut self) {
        // Reset task selection when changing projects
        let count = self.visible_tasks().len();
